...
public class TargetESP extends Module {
    ...
    private void onRenderWorldLast(MatrixStack matrices, Camera camera, float tickDelta) {
        ...
        if (mc.crosshairTarget != null && mc.crosshairTarget.getType() == HitResult.Type.ENTITY) {
            Entity crosshairEntity = ((EntityHitResult) mc.crosshairTarget).getEntity();
            if (crosshairEntity != mc.player && !(crosshairEntity instanceof ArmorStandEntity)) {
                if (crosshairEntity instanceof PlayerEntity player && player.isInvisible() && !hasArmor(player)) {
                    target = null;
                } else {
