...
public interface QuickLogger {
    static Text getPrefix() {
        MutableText brand = Text.literal("FGANG Visuals");
        brand.setStyle(brand.getStyle().withColor(Formatting.WHITE));

        MutableText prefix = Text.literal("");
        prefix.setStyle(brand.getStyle().withColor(Formatting.DARK_GRAY));
        prefix.append(brand);
        prefix.append(" -> ");
