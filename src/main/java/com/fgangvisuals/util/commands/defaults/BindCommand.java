...
public class BindCommand extends Command {
    ...
    public BindCommand(FGANGVisuals client) {
        super("bind");
        this.moduleStorage = client.getModuleStorage();
    }
    ...
    private void handleAddBind(String label, IArgConsumer args) throws CommandException {
        args.requireMin(2);
        List<ICommandArgument> list = new ArrayList<>(args.getArgs());
        String moduleName = list.subList(0, list.size() - 1).stream()
        ...
        for (int i = 0; i < list.size() - 1; i++) args.get();
    ...
    private void handleRemoveBind(IArgConsumer args) throws CommandException {
        args.requireMin(1);
        String moduleName = args.rawRest();
        Module module = moduleStorage.get(moduleName);
        if (module == null) {
            logDirect(Formatting.GRAY + "Модуль с названием " + Formatting.WHITE + moduleName + Formatting.GRAY + " не найден");
            return;
        }
        while (args.hasAny()) args.get();
        module.setKey(-1);
